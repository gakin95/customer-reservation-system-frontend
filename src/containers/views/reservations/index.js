...
import AlertDialog from "../../../components/modals/deleteModal";
import {
  errorNotification,
  successNotification,
...
const Reservations = () => {
  ...
  const { getReservationsByUserID, cancelReservation } =
    ReservationService();

  const onGetReservations = async () => {
    setIsLoading(true);
    const allVenus = await getReservationsByUserID(user.UserID);
  ...
  const getData = useCallback(() => {
    const result = ordersData.map((item, i) => {
      return {
        ...
        venueLocation: (
          <p className="text-NEUTRAL-_900 text-sm">{item?.venueLocation}</p>
        ),
        numberOfGuests: (
