...
const AddVenue = ({ closeModal, callback }) => {
  const { addVenue } = VenuesService();
  ...
  const submitForm = async (values) => {
    try {
      setloading(true);
      const result = await addVenue(values);
      setloading(false);
      if (result.status === "false" || result.status === false) {
        errorNotification(result.message);
      } else {
        successNotification("Venue added successfully");
        handleClose();
      }
    } catch (error) {
  ...
  return (
    <div>
      ...
      <h3 className="text-2xl mb-7">Add Venues</h3>
      <Formik
        initialValues={{
          venueName: "",
          venueType: "",
